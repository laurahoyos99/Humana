-- FIX PARA DUPLICADOS Y FALTA DE MESES DE GENERACION
with
base_fact as(
...
,base_super_clean as(
select distinct 
mes_vigente,b.* --except(segmentacion_cartera)
...
select distinct mes_vigente,clasificacion_contrato,sucursal, producto_principal,codigo_plan,forma_pago_contrato,canal_venta,segmentacion_cartera
,count(distinct contrato) as contratos--, count(distinct churner) as churners
from base_super_clean
