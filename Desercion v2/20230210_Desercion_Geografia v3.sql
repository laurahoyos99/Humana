...
select distinct 
--region,
mes_vigente,sala--,region
--mes_vigente,producto_principal,tenure
--,segmentacion_cartera,segmentacion_cliente,rango_etario,genero_contratante,calificacion_riesgo
--,sucursal,region,region_manual,agrup_de_ciudades,ciudad
,count(distinct contrato) as contratos
--,count(distinct churner) as churners,count(distinct case when churn_type='VOLUNTARIO' then churner else null end) as vol,count(distinct case when churn_type='CARTERA' then churner else null end) as invol
--,sum(afiliados) as total_afiliados
,sum(primas_ag) as total_primas
from union_churn_clean
where extract(year from mes_vigente)=2022
