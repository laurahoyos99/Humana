...
,cruce as(
select r.*
,case when date_diff(fecha_churn,fecha_reembolso,day)<=90 and fecha_churn> fecha_reembolso then 1 else 0 end as churn_flag
,case when date_diff(fecha_churn,fecha_reembolso,day)<=90 and fecha_churn> fecha_reembolso then r.contrato_r else null end as churner
,case when date_diff(fecha_churn,fecha_reembolso,day)<=90 and fecha_churn> fecha_reembolso then fecha_churn else null end as fecha_churn
