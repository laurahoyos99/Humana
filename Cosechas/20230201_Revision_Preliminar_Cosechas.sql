...
--select distinct segmentacion_cartera from base_clean
...
select distinct 
--Fecha_Inicio_Contrato,right(Fecha_Inicio_Contrato,7)--,parse_date(Fecha_Inicio_Contrato,'%d/%m/%Y') --
base_month,right(Fecha_Inicio_Contrato,7)
--date_trunc(date(parse_date(Fecha_Inicio_Contrato,'%d/%m/%Y')),month) as Inicio_Contrato
,count(distinct contrato) as contratos, count(distinct churner) as churners
from clasificacion_etario
where segmentacion_cartera='VENTAS NUEVAS'
