...
select distinct f.*,b.sala
from flag_modelo f inner join base_super_clean b on safe_cast(f.contrato as string)=b.contrato 
)
select distinct comp_flag_plan,sala,count(distinct f.contrato) as contratos,sum(dependientes) as afiliados,sum(prima_actual) as primas_reales, sum(prima_estimada) as primas_estimadas
from union_base_upsell f 
