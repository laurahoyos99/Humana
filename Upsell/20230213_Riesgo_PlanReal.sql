with
base_fact as(
SELECT distinct date(concat(ano_generacion,'-',mes_generacion,'-','01')) as base_month
,case when left(codigo_plan,6)='MH 150' then 'MH-150' when left(codigo_plan,6)='MH 80.' then 'MH-80' when left(codigo_plan,6)='MH 50.' then 'MH-50' when left(codigo_plan,6)='MH 30.' then 'MH-30' when left(codigo_plan,6)='PH 50.' then 'PH-50' when left(codigo_plan,6)='PH 30.' then 'PH-30' when left(codigo_plan,6)='PH 15.' then 'PH-15'
else codigo_plan end as codigo_plan_adj
,* except(mes_generacion,ano_generacion)
from `bustling-psyche-375313.humana_oval_2023.20230131_Base_Individual_Completa_2020_2022`  --limit 10
)
--select distinct codigo_plan,codigo_plan_adj,COUNT(DISTINCT CONTRATO)--,right(codigo_plan,6)
--from base_fact
...
select distinct mes_vigente,producto_principal,tenure
,segmentacion_cartera
--,segmentacion_cliente,rango_etario,genero_contratante
,codigo_plan_adj,calificacion_riesgo
